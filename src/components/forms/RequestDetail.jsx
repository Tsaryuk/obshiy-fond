...
function RequestDetail({ request, members, meId, onAcceptBid, onDeclineBid, onBid, onClose, onChatWith, T }) {
  ...
  return <Sheet onClose={onClose}>
    <div style={{display:"flex",gap:12,marginBottom:14}}>
      <div style={{width:46,height:46,borderRadius:12,background:"var(--color-border)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>
        {CAT_ICONS[request.category]||"🙋"}</div>
      <div><div style={{fontWeight:700,fontSize:16}}>{request.title}</div>
        <div style={{fontSize:13,color:"var(--color-text-tertiary)",marginTop:3}}>{request.desc}</div>
        {request.budget&&<div style={{fontSize:12,color:"#fbbf24",marginTop:4}}>Бюджет: до {cur(request.budget)}</div>}
      </div>
    </div>
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,padding:"8px 12px",background:"var(--color-border)",borderRadius:10}}>
      <Avatar member={author} size={22} />
      <span style={{fontSize:13,color:"var(--color-text-secondary)"}}>Запрос от {author.name}</span>
      <span style={{fontSize:11,color:"var(--color-text-faint)",marginLeft:"auto",fontFamily:"monospace"}}>{request.date}</span>
    </div>
    <SL>Предложения ({request.bids.length})</SL>
    {request.bids.length===0&&<div style={{textAlign:"center",color:"var(--color-text-faint)",padding:"16px 0",marginBottom:8}}>Пока никто не откликнулся</div>}
    <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
      {request.bids.map(bid=>{
        const bidder=findM(members,bid.from);
        const ok=bid.status==="accepted",dec=bid.status==="declined";
        return <div key={bid.id} style={{background:"var(--color-surface)",border:`1px solid ${ok?"#4ade8040":dec?"#33415560":"var(--color-border)"}`,
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            ...
            <span style={{fontSize:16,fontWeight:700,color:ok?"#4ade80":"var(--color-text-primary)"}}>{cur(bid.price)}</span>
          </div>
          {bid.note&&<div style={{fontSize:12,color:"var(--color-text-tertiary)",marginBottom:8,fontStyle:"italic"}}>«{bid.note}»</div>}
    ...
    {!isMe&&onChatWith&&<PB v="ghost" s={{marginTop:8}} onClick={()=>{onChatWith(request.member,`💬 По запросу «${request.title}»`);onClose();}}>✉️ Написать автору</PB>}
    ...
    {bidding&&<div style={{marginTop:10,padding:"14px",background:"var(--color-surface)",borderRadius:14,border:"1px solid var(--color-border)"}}>
      ...
      <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:10}}>
        <input type="number" min="0" value={bidPrice} onChange={e=>setBidPrice(e.target.value)} placeholder={`Сколько ${CUR.plural}`}
          style={{flex:1,background:"var(--color-input)",border:"1px solid var(--color-border)",borderRadius:10,color:"var(--color-text-primary)",padding:"11px 14px",fontSize:16,fontFamily:"inherit",outline:"none"}} />
        {request.budget&&<div style={{fontSize:12,color:"var(--color-text-tertiary)",textAlign:"right",flexShrink:0}}>
          <div>бюджет</div><div style={{color:"#fbbf24",fontWeight:600}}>{cur(request.budget)}</div></div>}
      </div>
      <SL>Комментарий</SL><FI value={bidNote} onChange={setBidNote} placeholder="Что предлагаешь…" multi />
