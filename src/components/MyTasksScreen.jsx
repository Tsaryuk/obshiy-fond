...
function MyTasksScreen({ meId, members, transactions, requests, T, onBack, onConfirmTx, onCancelTx, onMarkDone, onCancelRequest, onSelectMember, onOpenReq, reviews, onReview }) {
  ...
  function TxCard({ tx, role, reviews, onReview, meId: txMeId }) {
    ...
    return <div style={{background:"var(--color-surface)",border:"1px solid var(--color-border)",borderRadius:12,padding:"12px 14px",marginBottom:8}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div style={{flex:1}}>
          <div style={{fontWeight:600,fontSize:14,color:"var(--color-text-primary)",marginBottom:4}}>{tx.what}</div>
          <div style={{display:"flex",alignItems:"center",gap:7,fontSize:12,color:"var(--color-text-tertiary)"}}>
            <Avatar member={other} size={18}/>
            <span onClick={()=>onSelectMember(other.id)} style={{cursor:"pointer",color:"var(--color-accent)"}}>{other.name}</span>
            <span style={{fontSize:11,fontFamily:"monospace",marginLeft:4,color:"var(--color-text-faint)"}}>{tx.date}</span>
          </div>
        </div>
      ...
      {(tx.status==="active"||tx.status==="awaiting_confirm")&&<div style={{display:"flex",gap:7,marginTop:9,flexWrap:"wrap"}}>
        ...
        {role==="seller"&&tx.status==="active"&&!tx.reqId&&<div style={{fontSize:11,color:"var(--color-text-faint)",padding:"7px 10px",background:"var(--color-border)",borderRadius:8,flex:1,textAlign:"center"}}>в работе у покупателя</div>}
        ...
        {(tx.status==="active"&&!tx.reqId)||(tx.status==="active"&&role==="buyer"&&tx.reqId)
          ?<button onClick={()=>onCancelTx&&onCancelTx(tx.id)} style={{background:"var(--color-input)",border:"1px solid #7f1d1d",color:"#f87171",padding:"7px 10px",borderRadius:8,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>Отменить</button>
          :null}
      </div>}
      {tx.status==="confirmed"&&onReview&&(
        (reviews||[]).find(r=>r.txId===tx.id&&r.from===txMeId)
          ...
          : <button onClick={()=>onReview(tx)} style={{marginTop:7,width:"100%",background:"var(--color-input)",border:"1px solid var(--color-border)",color:"var(--color-text-tertiary)",padding:"5px",borderRadius:8,fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>⭐ Оставить отзыв</button>
      )}
    </div>;
  }

  const tabs = [{key:"get",l:"Жду получения"},{key:"give",l:"Мои обязательства"},{key:"reqs",l:"Мои запросы"}];
  return <div style={{background:"var(--color-bg)",minHeight:"100vh",color:"var(--color-text-primary)",fontFamily:"'DM Sans',sans-serif"}} {...swipe}>
    ...
    <div style={{padding:"12px 20px 0"}}>
      <div style={{fontSize:19,fontWeight:700,marginBottom:4,color:"var(--color-text-primary)"}}>Мои задачи</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:7,marginBottom:14}}>
        ...
        ].map((s,i)=><div key={i} style={{background:"var(--color-surface)",border:"1px solid var(--color-border)",borderRadius:12,padding:"8px 10px",textAlign:"center"}}>
          <div style={{fontSize:11,color:"var(--color-text-muted)",marginBottom:2}}>{s.l}</div>
          <div style={{fontSize:18,fontWeight:700,color:s.v>0?s.c:"var(--color-text-faint)"}}>{s.v}</div>
        </div>)}
      </div>
      <div style={{display:"flex",borderBottom:"1px solid var(--color-border)",marginBottom:14,overflowX:"auto"}}>
        {tabs.map(t=><button key={t.key} onClick={()=>setCtab(t.key)} style={{background:"none",border:"none",padding:"10px 0",marginRight:14,fontSize:12,
          fontWeight:ctab===t.key?600:400,color:ctab===t.key?"var(--color-text-primary)":"var(--color-text-muted)",
          borderBottom:ctab===t.key?`2px solid ${"var(--color-accent)"}`:"2px solid transparent",
      ...
      {ctab==="get"&&<div>
        ...
        {iGetDone.length>0&&<><div style={{fontSize:12,color:"var(--color-text-muted)",marginTop:12,marginBottom:8}}>✓ Получено ({iGetDone.length})</div>
          {iGetDone.map(tx=><TxCard key={tx.id} tx={tx} role="buyer" reviews={reviews} onReview={onReview} meId={meId}/>)}</>}
        {iGetActive.length===0&&iGetDone.length===0&&<div style={{textAlign:"center",color:"var(--color-text-faint)",padding:"28px 0",fontSize:13}}>Нет покупок</div>}
      </div>}
      {ctab==="give"&&<div>
        ...
        {myBidReqs.length>0&&<><div style={{fontSize:12,color:"#818cf8",fontWeight:600,marginTop:12,marginBottom:8}}>💬 Мои отклики</div>
          {myBidReqs.map(r=>{const bid=r.bids.find(b=>b.from===meId&&b.status==="pending");const auth=findM(members,r.member);
            return <div key={r.id} style={{background:"var(--color-surface)",border:"1px solid var(--color-border)",borderRadius:12,padding:"12px 14px",marginBottom:8}}>
              <div style={{fontWeight:600,fontSize:14,color:"var(--color-text-primary)",marginBottom:4}}>{r.title}</div>
              <div style={{display:"flex",alignItems:"center",gap:8,fontSize:12,color:"var(--color-text-tertiary)",marginBottom:8}}>
                <Avatar member={auth} size={18}/><span onClick={()=>onSelectMember(auth.id)} style={{cursor:"pointer",color:"var(--color-accent)"}}>{auth.name}</span>
        ...
        {iGiveDone.length>0&&<><div style={{fontSize:12,color:"var(--color-text-muted)",marginTop:12,marginBottom:8}}>✓ Выполнено ({iGiveDone.length})</div>
          {iGiveDone.map(tx=><TxCard key={tx.id} tx={tx} role="seller" reviews={reviews} onReview={onReview} meId={meId}/>)}</>}
        {iGiveActive.length===0&&iGiveDone.length===0&&myBidReqs.length===0&&<div style={{textAlign:"center",color:"var(--color-text-faint)",padding:"28px 0",fontSize:13}}>Нет обязательств</div>}
      </div>}
      {ctab==="reqs"&&<div>
        {myReqs.length===0&&<div style={{textAlign:"center",color:"var(--color-text-faint)",padding:"28px 0",fontSize:13}}>Нет запросов</div>}
        {myReqs.map(r=>{const pb=r.bids.filter(b=>b.status==="pending").length;
          return <div key={r.id} style={{background:"var(--color-surface)",border:`1px solid ${r.status==="closed"?"#4ade8040":pb>0?"#f9713040":"var(--color-border)"}`,
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
              <div style={{fontWeight:600,fontSize:14,color:"var(--color-text-primary)",flex:1}}>{r.title}</div>
            ...
            <div style={{fontSize:12,color:"var(--color-text-tertiary)",marginBottom:5,lineHeight:1.4}}>{r.desc}</div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:6}}>
              ...
              {r.status==="open"&&pb===0&&<button onClick={e=>{e.stopPropagation();onCancelRequest&&onCancelRequest(r.id);}}
                ...
                Отменить</button>}
              {r.status==="cancelled"&&<span style={{fontSize:11,color:"var(--color-text-faint)"}}>отменён</span>}
            </div>
          </div>;})}
      </div>}
    </div>
